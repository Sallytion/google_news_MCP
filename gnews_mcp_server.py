...
# Add CORS middleware to MCP app BEFORE creating Starlette app
mcp_app_with_cors = CORSMiddleware(
    mcp_app,
    allow_origins=["*"],
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
    expose_headers=["Mcp-Session-Id", "mcp-session-id"],  # Added lowercase version
)

# Create Starlette app - mount CORS-wrapped MCP app
app = Starlette(
    routes=[
        Route("/", root_handler),
        Mount("", app=mcp_app_with_cors),
    ],
    lifespan=lifespan
)
